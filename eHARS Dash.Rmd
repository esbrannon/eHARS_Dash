...
aggdata2 <- function(varlst, years){
  data0 = ehars[which(ehars$race %in% input$race_chkbx & ehars$age_cat %in% input$age_cat_chkbx &
                        ehars$trans_categ %in% input$trans_categ_chkbx),]
  data = data0[varlst]
  varlst2 <- paste("list(", paste("data$", varlst[2:length(varlst)], collapse=", ", sep=""), ")", sep="")
  data2 <- aggregate(data$count, by=eval(parse(text=varlst2)), FUN=sum)
  names(data2) = c(varlst[2:length(varlst)], varlst[1])
...
data3 <- reactive({
  vars = c("count", "yr_hiv","race", "age_cat", "trans_categ", "birth_sex")
  vars = vars[!is.na(vars)]
  data <- aggdata2(vars, c(input$year1, input$year2)) 
  data$colgroup <- paste(data$race, data$age_cat, data$trans_categ)
  data
  })
